...
from keyboards.pegination_kb import create_pagination_keyboard
...
async def process_help_command(message: Message):
    await message.anwer(LEXICON['/help'])
@router.message(Command(commands='beginning'))
async def process_beginning_commmand(message: Message):
...
async def process_forward_press(message: Message):
...
async def process_page_press(callback: CallbackQuery):
    ...
    await callback.answer('Cтраница добавлена в закладки!')
...
async def process_del_boolmark_press(callback: CallbackQuery):
    users_db[callback.from_user.id]['bookmarks'].remove(int(callback.data[:-3]))
    if users_db[callback.from_user.id]['bookmarks']:
        await callback.message.edit_text(
            text=LEXICON['/bookmarks'],
            reply_markup=create_edit_keyboard(
                *users_db[callback.from_users.id]["bookmarks"]))
    else:
